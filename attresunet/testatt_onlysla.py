...
x_train = x_train1.reshape(m, n, k, 1)


x_label = data_label.label.data[:]
mt,nt,kt,qt = np.shape(x_train)
# ######### normalization
pixels = x_train[:, :, :,:].ravel()  # flatten
x_max = max(pixels)
x_min = min(pixels)
x_train_old = (x_train-x_min)/(x_max-x_min)
...
# dice_coef_anti,dice_coef_cyc,mean_dice_coef, mean_iou, 'categorical_accuracy']
# eddynet = models.load_model('atteddynetssta.h5', custom_objects={'dice_coef_anti':dice_coef_anti,'dice_coef_cyc':dice_coef_cyc,'mean_dice_coef':mean_dice_coef,'mean_IOU':mean_iou,'accuracy':'categorical_accuracy'})

eddynet = models.load_model('../output/model_save/eddyatt_onlysla.h5',custom_objects={'dice_coef_loss':dice_coef_loss,'dice_coef_anti':dice_coef_anti,'dice_coef_cyc':dice_coef_cyc,'dice_coef_nn':dice_coef_nn,'mean_dice_coef':mean_dice_coef,'mean_iou':mean_iou,'InstanceNormalization': InstanceNormalization})
# eddynet.summary()
opt = optimizers.Adam(learning_rate=0.001, decay=1e-7)
eddynet.compile(optimizer=opt, loss=dice_coef_loss,
                metrics=[dice_coef_anti, dice_coef_cyc, mean_dice_coef, mean_iou, 'accuracy'])
tf.config.experimental_run_functions_eagerly(True)
loss = eddynet.evaluate(x_train_new, x_train_label, batch_size=5)
print('/nloss', loss)
# print('dice_coef', dice_coef)
# savemat('../evaluate_result/atteddynetzb_err15v2.mat',{'nloss':loss, 'dice_coef':dice_coef})
eddy = eddynet.predict(x_train_new, batch_size=5)
eddy = eddy.argmax(2).reshape(len(x_train_new), HEIGHT, WIDTH)
savemat('../output/detection_result/eddyatt_onlysla.mat', {'eddy': eddy,'evaluate':loss})
