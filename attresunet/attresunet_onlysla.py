...
HEIGHT = 80
WIDTH = 160
nbClass = 3 # Number to be classified
...
def double_conv_layer(x, filter_size, size, dropout, batch_norm=True):
    ...
    SAME padding
    selu nonlinear activation function
    :param x: input
    ...
    conv = layers.Activation('selu')(conv)
    conv = layers.Conv2D(size, (filter_size, filter_size), padding='same')(conv)
    if batch_norm is True:
        conv = InstanceNormalization(axis=axis)(conv)
    conv = layers.Activation('selu')(conv)
    if dropout > 0:
        conv = layers.AlphaDropout(dropout)(conv)
...
def SE_block(x, out_dim, ratio, name, batch_norm=True):
    ...
    x_e = layers.Activation('selu')(x_e)
    if batch_norm:
...
def Attention_ResUNet_PA(dropout_rate=0.05, batch_norm=True):
    '''
    ...
    # print(conv_final)
    conv_final = layers.Reshape((HEIGHT * WIDTH, OUTPUT_MASK_CHANNEL))(conv_final)
...
data_train = xr.open_dataset('../data/eddy_sla.nc') # load data
...
eddynet.save('../output/model_save/eddyatt_onlysla.h5')
