...
def basic_Block(input, out_filters, strides=(1, 1), with_conv_shortcut=False):
    ...
    if with_conv_shortcut:
        ...
        residual = InstanceNormalization(axis=3)(residual)
        x = add([x, residual])
    else:
        x = add([x, input])
...
def danet_resnet101(height, width, channel, classes):
    ...
    conv1_1 = InstanceNormalization(axis=3)(conv1_1)
    ...
    reduce_conv5_3 = InstanceNormalization(axis=3)(reduce_conv5_3)
    ...
    pam = InstanceNormalization(axis=3)(pam)
    ...
    cam = InstanceNormalization(axis=3)(cam)
    ...
    conv_final = Reshape((height * width, classes))(activation)
    model = Model(inputs=input, outputs=conv_final)
    return model
